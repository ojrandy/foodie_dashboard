...
export function Sidebar() {
  ...
  const content = (
    <>
      ...
      <div className="absolute top-0 left-full w-48 h-full pointer-events-none sidebar-bleed hidden md:block" />
      <div className="absolute top-0 left-full ml-48 w-24 h-full pointer-events-none sidebar-bleed opacity-40 hidden md:block" />
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              exit={{ x: -256 }}
              transition={{ duration: 0.2, ease: "easeInOut" }}
              className="fixed inset-y-0 left-0 z-50 w-64 border-r border-border/40 glass flex-col flex md:hidden h-screen overflow-visible"
            >
              {content}
            </motion.aside>
          </>
